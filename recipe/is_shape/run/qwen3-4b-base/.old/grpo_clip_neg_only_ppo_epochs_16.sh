...
# ============================================================================
# GRPO Clip Asymmetric: Only clip for negative advantages (A < 0)
#
# Key features:
#   - A > 0 (good samples): Use raw ratio (no clipping) for full learning signal
#   - A < 0 (bad samples): Apply PPO clipping to prevent over-correction
#
# Motivation:
#   - For positive samples: Allow full IS correction to properly learn good actions
#   - For negative samples: Clipping prevents too aggressive avoidance updates
# ============================================================================

TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)

ALGO="grpo_clip_neg_only"
OBJECT="math"
DATASET="deepscaler"
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    +actor_rollout_ref.actor.policy_loss.grpo_clip_asymmetric.mode=clip_neg_only \
