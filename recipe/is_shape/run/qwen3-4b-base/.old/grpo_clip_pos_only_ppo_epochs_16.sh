...
# ============================================================================
# GRPO Clip Asymmetric: Only clip for positive advantages (A > 0)
#
# Key features:
#   - A > 0 (good samples): Apply PPO clipping to prevent over-optimization
#   - A < 0 (bad samples): Use raw ratio (no clipping) for full gradient signal
#
# Motivation:
#   - For positive samples: Clipping prevents too aggressive updates on good actions
#   - For negative samples: Allow full IS correction to properly avoid bad actions
# ============================================================================

TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)

ALGO="grpo_clip_pos_only"
OBJECT="math"
DATASET="deepscaler"
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=grpo_clip_asymmetric \
    +actor_rollout_ref.actor.policy_loss.grpo_clip_asymmetric.mode=clip_pos_only \
    +actor_rollout_ref.actor.policy_loss.grpo_clip_asymmetric.clip_ratio=0.2 \
