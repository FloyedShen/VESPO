...
# ============================================================================
# Harmonic IS: IS-Reshape v10 with Harmonic Weighting
#
# Key features:
#   - MSE minimization framework → Defensive IS → Harmonic weighting
#   - Gradient weight: φ(w) = w / (w + λ)
#   - Objective function: f(w) = ln(λw + 1)
#   - Connects to JSD (GAN discriminator) when λ = 1
#
# Properties:
#   - Bounded: φ ∈ (0, 1) — prevents gradient explosion
#   - Concave: φ'' < 0 — automatic variance control
#   - Monotonic: φ' > 0 — preserves IS principle
#   - Unified: Single formula, no need to distinguish A > 0 vs A < 0
#
# Behavior:
#   - w << 1: φ ≈ w/λ (linear, IS-like correction)
#   - w >> 1: φ → 1 (saturate, SFT-like stability)
#   - w = 1:  φ = 1/(1+λ) (baseline)
#
# Hyperparameters:
#   - lambda_: Mixing parameter
#       λ → 0: more like IS (aggressive)
#       λ = 1: JSD form (recommended)
#       λ → ∞: more like SFT (conservative)
# ============================================================================
...
EXP_NAME="${ALGO}_${OBJECT}_${DATASET}_${MODEL}_ppo_epochs_${PPO_EPOCHS}_lambda_0.1"
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=harmonic_is \
    +actor_rollout_ref.actor.policy_loss.harmonic_is.lambda_=0.1 \
