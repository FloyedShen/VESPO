...
# ============================================================================
# IB-IS: Information Bottleneck Importance Sampling (IS-Reshape v9)
#
# Key features:
#   - Views IS weight processing as information compression problem
#   - Uses Softplus for entropy-regularized soft constraints (Fenchel-Legendre dual)
#   - Asymmetric compression for positive/negative advantages
#
# Core formulas:
#   A > 0: rho_smooth = C - tau * Softplus((C - rho) / tau)  [upper bound]
#   A < 0: rho_smooth = -C + tau * Softplus((C + rho) / tau) [lower bound]
#
# Properties:
#   - Bounded weights (unlike w^gamma which is unbounded)
#   - Smooth and differentiable everywhere
#   - Asymmetric treatment matches learning objectives
#   - Clear information-theoretic interpretation
#
# Hyperparameters:
#   - bandwidth: Information bandwidth C (controls truncation position)
#   - temperature: Smoothness tau (smaller = harder truncation)
# ============================================================================
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=ib_is \
    +actor_rollout_ref.actor.policy_loss.ib_is.bandwidth=0.5 \
    +actor_rollout_ref.actor.policy_loss.ib_is.temperature=1.0 \
