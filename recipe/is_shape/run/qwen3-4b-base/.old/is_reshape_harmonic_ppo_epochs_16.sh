...
# ============================================================================
# IS Reshape Harmonic: Asymmetric bounded weight functions
#
# Key features:
#   - A > 0: h+(w) = 1/(w + c)    (convex, decaying, bounded)
#   - A < 0: h-(w) = w/(1 + cw)   (concave, growing, bounded)
#
# Properties:
#   - Both functions bounded: max weight = 1/c (e.g., 10 for c=0.1)
#   - Meet at w=1: h+(1) = h-(1) = 1/(1+c)
#   - Automatic variance control (no exploding weights)
#   - Soft trust region (implicit clipping)
#
# Behavior:
#   h+(w) for A > 0:
#     - w < 1 (new good samples): high weight ~1/c
#     - w > 1 (known good samples): low weight → 0
#
#   h-(w) for A < 0:
#     - w < 1 (avoided bad samples): low weight → 0
#     - w > 1 (un-avoided bad samples): high weight ~1/c
# ============================================================================

TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)

ALGO="is_reshape_harmonic"
OBJECT="math"
DATASET="deepscaler"
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=is_reshape_harmonic \
    +actor_rollout_ref.actor.policy_loss.is_reshape_harmonic.c=0.1 \
    +actor_rollout_ref.actor.policy_loss.is_reshape_harmonic.clip_ratio=100.0 \
