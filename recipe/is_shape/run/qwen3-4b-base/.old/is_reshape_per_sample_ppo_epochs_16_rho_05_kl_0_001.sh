...
# ============================================================================
# IS Reshape Per-Sample: batch_size=256, ppo_epochs=16
# Per-sample gamma computation (each sample gets its own optimal gamma)
# Data Reuse: Each batch is trained 16 times
# ============================================================================

TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)

ALGO="is_reshape_per_sample"
OBJECT="math"
DATASET="deepscaler"
...
EXP_NAME="${ALGO}_${OBJECT}_${DATASET}_${MODEL}_ppo_epochs_${PPO_EPOCHS}_rho_0.5_kl_0.001"
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=is_reshape_per_sample \
    +actor_rollout_ref.actor.policy_loss.is_reshape_per_sample.rho_min=0.5 \
    +actor_rollout_ref.actor.policy_loss.is_reshape_per_sample.gamma_min=0.05 \
    +actor_rollout_ref.actor.policy_loss.is_reshape_per_sample.gamma_max=1.0 \
    +actor_rollout_ref.actor.policy_loss.is_reshape_per_sample.gamma=null \
    +actor_rollout_ref.actor.policy_loss.is_reshape_per_sample.clip_weight=true \
    +actor_rollout_ref.actor.policy_loss.is_reshape_per_sample.clip_threshold=10.0 \
