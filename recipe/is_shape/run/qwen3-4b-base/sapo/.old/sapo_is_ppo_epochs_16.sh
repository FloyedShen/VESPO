...
# ============================================================================
# SAPO-IS: SAPO with IS-Reshape Aware Asymmetric Gating
#
# Key improvement over standard SAPO:
#   - A > 0: Flipped sigmoid (concave-like) → amplify w<1, suppress w>1
#   - A < 0: Original sigmoid (convex-like) → suppress w<1, amplify w>1
#
# Gate functions:
#   gate_pos(w) = 2 * sigmoid(-τ_pos * (w - 1))  # Concave-like
#   gate_neg(w) = 2 * sigmoid(τ_neg * (w - 1))   # Convex-like
#
# Properties:
#   - Both bounded in [0, 2]
#   - Both equal 1 at w=1 (normalized)
#   - Matches IS-Reshape theory for optimal variance-bias tradeoff
# ============================================================================
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=sapo_is \
    +actor_rollout_ref.actor.policy_loss.sapo_is.tau_pos=1.0 \
    +actor_rollout_ref.actor.policy_loss.sapo_is.tau_neg=1.0 \
