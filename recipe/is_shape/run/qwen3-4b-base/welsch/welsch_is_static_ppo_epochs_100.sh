...
# ============================================================================
# Welsch IS Static: IS-Reshape v12 with Static Lambda
#
# Key insight: Welsch uses Gaussian-like trust region:
#   gate = exp(-0.5 * λ * (w - 1)²)
#   φ = w * gate
#
# With urgency_scale=0, lambda becomes static:
#   λ(A) = lambda_base * exp(-0 * A) = lambda_base (constant)
#
# This provides:
#   - Stable training baseline
#   - Symmetric treatment of A>0 and A<0
#   - Soft trust region centered at w=1
#
# Comparison with Cauchy IS Static:
#   - Welsch Gaussian gate decays faster than Arctan
#   - More aggressive outlier suppression
#   - Potentially more stable but less sample-efficient
# ============================================================================

TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)

ALGO="welsch_is_static"
OBJECT="math"
DATASET="deepscaler"
...
EXP_NAME="${ALGO}_${OBJECT}_${DATASET}_${MODEL}_ppo_epochs_${PPO_EPOCHS}_l_100"
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    +actor_rollout_ref.actor.policy_loss.welsch_is.lambda_base=100.0 \
    +actor_rollout_ref.actor.policy_loss.welsch_is.urgency_scale=0.0 \
    +actor_rollout_ref.actor.policy_loss.welsch_is.lambda_min=0.1 \
    +actor_rollout_ref.actor.policy_loss.welsch_is.lambda_max=1000.0 \
