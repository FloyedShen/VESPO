...
# ============================================================================
# Cauchy IS (Static Mode): IS-Reshape v11 with Fixed λ
#
# Static mode uses a single fixed λ value instead of dynamic λ(A).
# This is simpler and recommended for initial experiments.
#
# Core formula:
#   f(w) = (1/√λ) · arctan(√λ · w)
#   φ(w) = w / (1 + λ·w²)    [effective gradient weight]
#
# λ controls the bias-variance tradeoff:
#   - λ → 0: φ → w (pure IS, high variance)
#   - λ → ∞: φ → 1/w (inverse, very conservative)
#   - λ = 1: φ = w/(1+w²) (balanced, recommended starting point)
#
# Recommended λ values:
#   - λ = 0.5: More aggressive (closer to IS)
#   - λ = 1.0: Balanced (recommended)
#   - λ = 2.0: More conservative (more variance control)
# ============================================================================
...
EXP_NAME="${ALGO}_${OBJECT}_${DATASET}_${MODEL}_ppo_epochs_${PPO_EPOCHS}_static_l_100"
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=cauchy_is \
    +actor_rollout_ref.actor.policy_loss.cauchy_is.lambda_base=100. \
    +actor_rollout_ref.actor.policy_loss.cauchy_is.urgency_scale=0.0 \
    +actor_rollout_ref.actor.policy_loss.cauchy_is.lambda_min=0.01 \
    +actor_rollout_ref.actor.policy_loss.cauchy_is.lambda_max=1000.0 \
