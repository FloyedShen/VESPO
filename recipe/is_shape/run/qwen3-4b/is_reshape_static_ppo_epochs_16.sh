...
# ============================================================================
# IS Reshape Static: Fixed symmetric γ values (0.5 / 1.5)
#
# Key features:
#   - γ+ = 0.5 fixed for positive samples (A > 0)
#   - γ- = 1.5 fixed for negative samples (A < 0)
#   - Symmetric around 1: 0.5 + 1.5 = 2, midpoint = 1
#   - No adaptive computation - simplest baseline for asymmetric IS reshape
#
# Weight functions:
#   - γ+ = 0.5: w^0.5 = √w (concave)
#   - γ- = 1.5: w^1.5 = w√w (convex)
# ============================================================================

TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)

ALGO="is_reshape_static"
OBJECT="math"
DATASET="deepscaler"
...
python3 -u -m recipe.is_shape.code.main_ppo \
    ...
    actor_rollout_ref.actor.policy_loss.loss_mode=is_reshape_static \
    +actor_rollout_ref.actor.policy_loss.is_reshape_static.gamma_pos=0.5 \
    +actor_rollout_ref.actor.policy_loss.is_reshape_static.gamma_neg=2. \
    +actor_rollout_ref.actor.policy_loss.is_reshape_static.clip_weight=true \
    +actor_rollout_ref.actor.policy_loss.is_reshape_static.clip_threshold=10.0 \
