...
class OptimalSamplingModel:
    ...
    def __init__(
        ...
        target_top_k: int = -1,
        target_top_p: float = 1.0,
        force_target_for_special_tokens: bool = False,
        force_target_for_first_token: bool = False,
        device: str = "cuda",
    ...
    def _init_transformers(self, model_theta_path: str, model_t_path: Optional[str], **kwargs):
        ...
        # ========================================
        # Detect if path is local or from hub
        import os
        is_local_theta = os.path.exists(model_theta_path) if model_theta_path else False

        print(f"Loading tokenizer for π_θ from {model_theta_path}...")
        self.tokenizer_theta = AutoTokenizer.from_pretrained(
            model_theta_path,
            trust_remote_code=True,
            local_files_only=is_local_theta
        )
        ...
        if model_t_path is None or model_t_path == model_theta_path:
            ...
            is_local_t = os.path.exists(model_t_path) if model_t_path else False

            print(f"Loading tokenizer for π_t from {model_t_path}...")
            self.tokenizer_t = AutoTokenizer.from_pretrained(
                model_t_path,
                trust_remote_code=True,
                local_files_only=is_local_t
            )
    ...
    def _generate_transformers(
        ...
        **kwargs
        ...
        if use_kv_cache:
            ...
            tqdm_desc = kwargs.get('tqdm_desc', 'Generating')
            for step in tqdm(range(max_new_tokens), desc=tqdm_desc):
