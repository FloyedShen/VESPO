    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            ...
                            # Compute entropy metrics from entropys
                            if "entropys" in old_log_prob.batch:
                                ...
                                batch_metrics["actor/entropy"] = entropy_agg.detach().item()
                                old_log_prob.batch.pop("entropys")
                            batch = batch.union(old_log_prob)
                            # Calculate debug metrics if rollout_log_probs available
                            if "rollout_log_probs" in batch.batch.keys():
                                from verl.utils.debug.metrics import calculate_debug_metrics
                                batch_metrics.update(calculate_debug_metrics(batch))
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # Compute rollout correction weights if in decoupled mode
                        if (
                            ...
                            batch_metrics.update(is_metrics)
                ...
                batch_metrics.update(compute_variance_proxy_metrics(batch=batch))

                # Handle per-policy-update metrics logging
                if self.config.trainer.critic_warmup <= (rollout_batch_idx + 1):
                    actor_metrics = actor_output.meta_info.get("metrics", {})

                    # Extract update_metrics from meta_info (new location)
                    # or from metrics["__update_metrics__"] (legacy location)
                    all_update_metrics = actor_output.meta_info.get("update_metrics", [])
                    if not all_update_metrics and "__update_metrics__" in actor_metrics:
